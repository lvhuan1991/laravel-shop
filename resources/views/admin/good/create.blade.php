                    ...
                    <div class="row">
                        <div class="col-6">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input type="text" name="title" value="{{old('title')}}" placeholder="请输入商品名称"
                                           class="form-control">
                                </div>
                            </div>
                            <div class="form-group row">
                        ...
                        <div class="col-6" id="app">
                            <div class="card" v-for="(v,k) in specs">
                                <div class="card-body">
                                    ...
                                    <button @click="del(k)" class="btn btn-danger btn-sm pull-right" type="button">删除
                                    </button>
                                </div>
                            </div>
                            <div class="">
    ...
    <script>
        layui.use(['upload', 'layedit'], function () {
            ...
            upload.render({
                ...
                , size: "{{my_config('upload.upload_size',2000000)}}" //限制文件大小第二个参数是默认值，单位 KB
                , accept: 'images' //{指定允许上传时校验的文件类型，可选：images（图片）、file（所有文件）、video（视频）、audio（音频）}
                , acceptMime: "{{my_config('upload.upload_accept_mime','image/png,image/jpg,image/gif,image/bmp,image/jpeg')}}" //规定打开文件选择框时，筛选出的文件类型
                , exts: "{{my_config('upload.upload_type','png|jpg|gif|bmp|jpeg')}}"  //允许上传的文件后缀。一般结合 accept 参数类设定
